...
import type { FormData, WebhookResponse, EmailWithVerification, EmailVerificationResult, EmailWithConfidence, DomainProcessingResult, MultiDomainProgress } from '../types';
...
const useEmailForm = (): UseEmailFormReturn => {
  ...
  const validateForm = useCallback(() => {
    ...
    // For single domain processing, the server will automatically handle Perplexity API calls
    // For multi-domain processing, the server will handle all domains
    setDomainError('');
    return true;
  }, [formData, domainValidation, isDomainValidating]);
  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      ...
      // Only send webhook response if it exists (for manual mode)
      if (webhookResponse) {
      ...
      console.log('🔍 Domain being processed:', formData.domain);
      console.log('🔍 Webhook response exists:', !!webhookResponse);
  ...
  }, [formData, webhookResponse, validateForm]);
  ...
  const triggerDomainWebhook = useCallback(async () => {
    // This function can be implemented based on your webhook requirements
    // For now, it's a placeholder that doesn't throw an error
    console.log('Domain webhook triggered');
  }, []);

  const showWebhookResponse = useCallback(() => {
    // This function can be implemented to show webhook responses
    // For now, it's a placeholder that doesn't throw an error
    console.log('Showing webhook response');
  }, []);
